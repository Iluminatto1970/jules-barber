...
if(@count($res2) > 0){
	...
	if((string)$usuario === '0'){
		$data_cad_cliente = isset($res2[0]['data_cad']) ? $res2[0]['data_cad'] : '';
		$data_nasc_cliente = isset($res2[0]['data_nasc']) ? $res2[0]['data_nasc'] : '';
		$endereco_cliente = isset($res2[0]['endereco']) ? trim((string)$res2[0]['endereco']) : '';

		$query_auto = $pdo->prepare("SELECT id FROM agendamentos WHERE cliente = :cliente ORDER BY data ASC, hora ASC, id ASC LIMIT 1");
		$query_auto->bindValue(':cliente', $cliente);
		$query_auto->execute();
		$primeiro_agendamento = $query_auto->fetch(PDO::FETCH_ASSOC);
		$primeiro_id = $primeiro_agendamento ? (string)$primeiro_agendamento['id'] : '';

		if($data_cad_cliente == $data_lanc && ($data_nasc_cliente == '2000-01-01' || $data_nasc_cliente == '0000-00-00') && $endereco_cliente == '' && $primeiro_id === (string)$id){
			$cliente_cadastro_auto = true;
		}
	}
}else{
...
if($cliente_cadastro_auto){
	$badge_bg = '#1f8d4f';
	$badge_border = '#166a3a';
	$badge_icon = 'fa-clock-o';
	...
	$badge_cliente_auto = '<br><span style="display:inline-flex; align-items:center; gap:5px; margin-top:6px; padding:3px 9px; border-radius:999px; background:'.$badge_bg.'; border:1px solid '.$badge_border.'; color:#fff; font-size:10px; font-weight:700; letter-spacing:0.01em;"><i class="fa '.$badge_icon.'" aria-hidden="true"></i>Cliente cadastrado automaticamente</span>';
}
        		...
        		<hr style="margin-top:-2px; margin-bottom: 3px">                    
                    <div class="stats esc" align="center">
                      <span>
                       ...
                       <small> <span class="{$ocultar_cartoes}" style=""><img class="icon-rounded-vermelho" src="img/presente.jpg" width="20px" height="20px"></span> {$nome_cliente} (<i><span style="color:#061f9c">{$nome_serv}</span></i>){$badge_cliente_auto}</small></span>
                    </div>
                </div>
	        	</div>
HTML;
